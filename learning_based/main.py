...
def amplify(model_path, input_video, *, amplification=1.0, device="cpu", skip_frames=1):
    ...
    amp_video.write_videofile(output_path, codec='rawvideo')
    return output_path


def demo(model_path, input_video, output_video, *amplification_factors, device="cpu",
         skip_frames=1):
    amplified = []
...
def collage(output_video, *input_videos):
    ...
    final_clip.write_videofile(output_video, audio=False, codec ='rawvideo')
    return output_video
